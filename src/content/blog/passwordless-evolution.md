...
AuthMS's mfa-service still offers SMS OTP as an option (for backward compatibility with existing users), but recommends admins prioritize TOTP and Passkey.
...
- AuthMS's mfa-service already supports number-matching push notifications
...
### AuthMS Passkey Implementation

AuthMS has had built-in full WebAuthn RP support since 2025:
...
AuthMS fully supports every stage in this table—not because you need to deploy SMS OTP in 2026, but because you can choose the right combination of authentication methods for different user groups, different security levels, and different compliance requirements. The evolution from passwords to Passkey isn't a migration—it's a journey. AuthMS has prepared every stop along the way.
